...
def venues():
    ...
    for location in locations:
        ...
        query = db.session.query(Venue).filter(
            Venue.city == city_name, Venue.state == state_name).all()
        for result in query:
            group["venues"].append({
                "id": result.id,
                "name": result.name,
...
def show_venue(venue_id):
    ...
    past_show_query = Show.query.filter(
        Show.start_time < datetime.datetime.now(), Show.venue_id == venue_id)
    upcoming_show_query = Show.query.filter(
        Show.start_time >= datetime.datetime.now(), Show.venue_id == venue_id)
    data = {
        "id": result.id,
        "name": result.name,
        ...
        "past_shows": [],
        "upcoming_shows": [],
        "past_shows_count": past_show_query.count(),
        "upcoming_shows_count":  upcoming_show_query.count(),
    }

    for past_show in past_show_query.all():
        show = {
    ...
    for upcoming_show in upcoming_show_query.all():
        show = {
...
def create_venue_submission():
    ...
    try:
        ...
        db.session.rollback()
        flash('An error occurred. Veunue ' +
...
def show_artist(artist_id):
    ...
    for upcoming_show in upcoming_show_query.all():
        show = {
            "artist_id": upcoming_show.venue.id,
...
def shows():
    ...
    # TODO: replace with real venues data. - Check
    #       num_shows should be aggregated based on number of upcoming shows per venue.
    # data = [{
    #     "venue_id": 1,
    #     "venue_name": "The Musical Hop",
    ...
    #     "start_time": "2035-04-15T20:00:00.000Z"
    # }]
    shows = db.session.query(Show).join(Venue).join(
        Artist)
    data = []
    for show in shows:
        show = {
